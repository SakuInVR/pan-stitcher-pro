...
function renderPanorama(){
  const {spatial:base,anchors,axis,w,h,adjustments}=editorState,vertical=axis==='vertical',length=vertical?h:w;
  ...
  for(let i=0;i<spatial.length;i++){
    ...
    drawBand(frame,p,bandStart,bandEnd);
  }
...
function updateSeamGuide(){const guide=$('#seamGuide');if($('#seamEditor').hidden||!editorState){guide.hidden=true;return}const index=+$ ('#seamSelect').value,value=editorState.adjustments[index]||0,boundary=editorState.boundaries[index],vertical=editorState.axis==='vertical',scale=vertical?out.clientHeight/out.height:out.clientWidth/out.width;guide.hidden=false;guide.className=`seam-guide ${vertical?'vertical':'horizontal'}`;guide.style[vertical?'top':'left']=`${boundary*scale}px`;$('#seamGuideLabel').textContent=`境界 ${index+1}｜${value>0?'+':''}${value}px`}
function focusSeam(){const index=+$ ('#seamSelect').value,boundary=editorState.boundaries[index],wrap=$('.canvas-wrap'),scale=editorState.axis==='vertical'?out.clientHeight/out.height:out.clientWidth/out.width;updateSeamGuide();if(editorState.axis==='vertical')wrap.scrollTop=Math.max(0,boundary*scale-wrap.clientHeight/2);else wrap.scrollLeft=Math.max(0,boundary*scale-wrap.clientWidth/2)}
...
$('#closeSeamEditor').onclick=()=>{$('#seamEditor').hidden=true;$('#openSeamEditor').hidden=false;$('#seamGuide').hidden=true;$('#openSeamEditor').scrollIntoView({behavior:'smooth',block:'center'})};
